...
void quick_plotter(TString filename){
  TreeReader tree(filename);

  auto mgt = new TMultiGraph();
  mgt->SetTitle(TString::Format("Sample trigger channel 0 (First 20 events);Time [ns];Voltage [mV]"));
  ...
  */
  ...
  mgt->GetXaxis()->SetLimits(120,180);
  mgt->SetMinimum(-500);
  mgt->SetMaximum(200);
